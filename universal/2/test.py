...
# --- Configuration (MUST MATCH V2 TRAINING SCRIPT) ---
# Data paths
BASE_DATA_DIR = "../../data/time_series/1"
VALID_DIR = os.path.join(BASE_DATA_DIR, "VALIDATION")

# Model & Training Parameters (from V2 training script)
SEQ_LEN = 64
PRED_HORIZONS = [1, 3, 5]
...
# Architectural Params (from V2 training script)
SENSOR_INPUT_DIM = 1
...
MODEL_LOAD_PATH = "foundation_timeseries_model_v2.pth"
PREPROCESSOR_LOAD_PATH = "preprocessor_config_v2.npz"


# --- Helper: Positional Encoding (from V2 training script) ---
...
# --- Helper: CausalConv1D Block (from V2 training script) ---
...
# --- Data Handling (Replicated from V2 training script) ---
    ...
    def _load_and_prepare_data(self):
        ...
        for file_idx, fp in enumerate(self.file_paths):
            ...
            self.data_cache.append({
                ...
                "filepath": fp  # Store filepath for reference in test script
            })
...
# --- Model Architecture (Replicated from V2 training script) ---
...
class FoundationalTimeSeriesModelV2(nn.Module):
    ...
    def forward(self, x_features, sensor_mask):
        ...
        expanded_cross_sensor_context = cross_sensor_context.unsqueeze(2).expand(-1, -1, seq_len,
                                                                                 TRANSFORMER_D_MODEL)  # Use TRANSFORMER_D_MODEL here
...
def test_model():
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(valid_loader):
            ...
            last_known_values_batch = batch["last_known_values"].to(DEVICE)  # [B, MaxSensors]
            pred_delta_targets_gt = batch["pred_delta_targets"].to(DEVICE)  # [B, MaxSensors, NumPredHorizons]
            ...
            for i in range(num_samples_this_batch_to_print):
                ...
                last_known_values_sample = last_known_values_batch[i]  # [MaxSensors]
