...
MODEL_SAVE_PATH = "foundation_multitask_model_v3_moe_vectorized_ema_cost_sensitive.pth"  # Updated model name
PREPROCESSOR_SAVE_PATH = "foundation_multitask_preprocessor_v3_ema.npz"  # Updated preprocessor name

# Loss Function Parameters
HUBER_DELTA = 1.0
EMA_BETA = 0.9  # Smoothing factor for EMA weights

# Gamma for Focal Loss (Alpha will be derived dynamically)
...
class FocalLoss(nn.Module):
    def __init__(self, alpha=0.25, gamma=2.0, reduction='mean'):
        ...
        self.bce_with_logits = nn.BCEWithLogitsLoss(reduction='none')  # Always 'none' here, reduction handled outside

    def forward(self, inputs, targets):
        bce_loss = self.bce_with_logits(inputs, targets)
        pt = torch.exp(-bce_loss)
        # Calculate alpha_t dynamically based on targets
        # self.alpha_param is the weight for the positive class (target=1)
        ...
        # elif self.reduction == 'none': # Default behavior if not mean or sum
        return focal_term
...
def train_and_save_model():
    ...
    # Initialize EMA variables for dynamic loss weighting
    ema_pos_fail, ema_tot_fail = torch.tensor(0.0, device=DEVICE), torch.tensor(0.0, device=DEVICE)
    ema_pos_rca, ema_tot_rca = torch.tensor(0.0, device=DEVICE), torch.tensor(0.0, device=DEVICE)

    print("Starting multi-task training (Forecast, Fail, RCA) with EMA-weighted Cost-Sensitive Losses...")
    for epoch in range(EPOCHS):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with autocast(enabled=AMP_ENABLED):
                ...
                # --- Failure Loss (EMA weighted BCE + Focal) ---
                batch_pos_fail = fail_tgt.sum().float()
                batch_tot_fail = torch.tensor(fail_tgt.numel(), device=DEVICE, dtype=torch.float)

                ema_pos_fail = EMA_BETA * ema_pos_fail + (1 - EMA_BETA) * batch_pos_fail
                ema_tot_fail = EMA_BETA * ema_tot_fail + (1 - EMA_BETA) * batch_tot_fail
                pos_w_fail = (ema_tot_fail - ema_pos_fail) / (ema_pos_fail + 1e-6)
                # pos_w_fail = torch.clamp(pos_w_fail, min=0.1, max=10000.0) # Optional clamping

                alpha_fail = pos_w_fail / (1.0 + pos_w_fail)

                lf_bce = F.binary_cross_entropy_with_logits(fail_logits, fail_tgt, pos_weight=pos_w_fail.detach())

                focal_fail_loss_fn = FocalLoss(alpha=alpha_fail.item(), gamma=FAILURE_FOCAL_GAMMA, reduction='mean')
                lf_focal_term = focal_fail_loss_fn(fail_logits, fail_tgt)

                lf = lf_bce + FAILURE_FOCAL_BLEND_COEFF * lf_focal_term

                # --- RCA Loss (EMA weighted BCE + Focal) ---
                batch_pos_rca = (rca_tgt * sensor_m).sum().float()
                batch_tot_rca = sensor_m.sum().float().clamp(
                    min=1.0)  # Clamp to avoid div by zero if no sensor is active

                ema_pos_rca = EMA_BETA * ema_pos_rca + (1 - EMA_BETA) * batch_pos_rca
                ema_tot_rca = EMA_BETA * ema_tot_rca + (1 - EMA_BETA) * batch_tot_rca
                pos_w_rca = (ema_tot_rca - ema_pos_rca) / (ema_pos_rca + 1e-6)
                # pos_w_rca = torch.clamp(pos_w_rca, min=0.1, max=10000.0) # Optional clamping

                alpha_rca = pos_w_rca / (1.0 + pos_w_rca)

                lr_bce = F.binary_cross_entropy_with_logits(rca_logits, rca_tgt, weight=sensor_m,
                                                            pos_weight=pos_w_rca.detach())

                focal_rca_loss_fn = FocalLoss(alpha=alpha_rca.item(), gamma=RCA_FOCAL_GAMMA,
                                              reduction='none')  # Element-wise
                lr_focal_elements = focal_rca_loss_fn(rca_logits, rca_tgt)
            ...
            if batch_idx > 0 and batch_idx % (len(train_loader) // 10 if len(train_loader) >= 10 else 1) == 0:
                ...
                # Debug EMA weights
                # print(f"    EMA Fail: pos_w={pos_w_fail.item():.2f}, alpha={alpha_fail.item():.3f} | EMA RCA: pos_w={pos_w_rca.item():.2f}, alpha={alpha_rca.item():.3f}")
            ...
            with torch.no_grad():
                for batch_val in valid_loader:
                    ...
                    with autocast(enabled=AMP_ENABLED):
                        ...
                        # Failure Loss (Validation - use current EMA state, do not update EMA)
                        # Note: ema_pos_fail and ema_tot_fail reflect the state after last training batch
                        current_pos_w_fail = (ema_tot_fail - ema_pos_fail) / (ema_pos_fail + 1e-6)
                        # current_pos_w_fail = torch.clamp(current_pos_w_fail, min=0.1, max=10000.0)
                        current_alpha_fail = current_pos_w_fail / (1.0 + current_pos_w_fail)

                        lf_bce_val = F.binary_cross_entropy_with_logits(fail_logits_val, fail_tgt_val,
                                                                        pos_weight=current_pos_w_fail.detach())
                        focal_fail_loss_fn_val = FocalLoss(alpha=current_alpha_fail.item(), gamma=FAILURE_FOCAL_GAMMA,
                                                           reduction='mean')
                        lf_focal_val = focal_fail_loss_fn_val(fail_logits_val, fail_tgt_val)
                        lf_val = lf_bce_val + FAILURE_FOCAL_BLEND_COEFF * lf_focal_val

                        # RCA Loss (Validation - use current EMA state, do not update EMA)
                        current_pos_w_rca = (ema_tot_rca - ema_pos_rca) / (ema_pos_rca + 1e-6)
                        # current_pos_w_rca = torch.clamp(current_pos_w_rca, min=0.1, max=10000.0)
                        current_alpha_rca = current_pos_w_rca / (1.0 + current_pos_w_rca)

                        lr_bce_val = F.binary_cross_entropy_with_logits(rca_logits_val, rca_tgt_val,
                                                                        weight=sensor_m_val,
                                                                        pos_weight=current_pos_w_rca.detach())
                        focal_rca_loss_fn_val = FocalLoss(alpha=current_alpha_rca.item(), gamma=RCA_FOCAL_GAMMA,
                                                          reduction='none')
                        lr_focal_elements_val = focal_rca_loss_fn_val(rca_logits_val, rca_tgt_val)
...
if __name__ == '__main__':
    print(
        "--- Script Version: Multi-Task Foundational Model with Refactored MoE (Vectorized Soft Top-k, MMoE, Token RCA, Richer Router, Entropy Reg, EMA Cost-Sensitive) ---")
