...
SEQ_LEN = 64
PRED_HORIZONS = [1, 3, 5]
FAIL_HORIZONS = [3, 5, 10]  # Used by hysteresis & LogSumExp logic
...
MODEL_SAVE_PATH = "foundation_multitask_model_v3_moe_vectorized_ema_cost_sensitive_updated.pth"
PREPROCESSOR_SAVE_PATH = "foundation_multitask_preprocessor_v3_ema_updated.npz"

# Loss Function Parameters
HUBER_DELTA = 1.0
EMA_BETA = 0.9  # Smoothing factor for EMA weights (still used for Failure Focal Loss alpha)

# Gamma for Focal Loss (Alpha will be derived dynamically for Failure, statically for RCA)
FAILURE_FOCAL_GAMMA = 1.5
RCA_FOCAL_GAMMA = 1.0

# Blending coefficients for Focal Loss terms
FAILURE_FOCAL_BLEND_COEFF = 0.2
RCA_FOCAL_BLEND_COEFF = 0.1
...
# Change 3: Static class-weight for RCA
POS_W_RCA = 25.0  # tune 10-50

# Change 4: Label smoothing factor (applied as target * factor)
# The sketch rca_tgt = rca_tgt * 0.98 makes 1->0.98, 0->0. Hardcoding as per sketch.
LABEL_SMOOTH_FACTOR = 0.98

# Change 5: Hysteresis thresholds for post-processing
...
class FocalLoss(nn.Module):
    def __init__(self, alpha=0.25, gamma=2.0, reduction='mean'):
        ...
        self.alpha_param = alpha  # This will be the calculated alpha_t for the positive class
        self.gamma = gamma
    ...
    def forward(self, inputs, targets):
        ...
        # alpha_t is self.alpha_param for positive targets, (1-self.alpha_param) for negative targets
...
class FoundationalTimeSeriesModel(nn.Module):
    def __init__(self, model_max_sensors, seq_len,
                 ...
                 pred_horizons_len, fail_horizons_len,  # fail_horizons_len used by fail_head
        ...
        self.fail_head = nn.Linear(moe_output_dim, fail_horizons_len)  # Output size based on len(FAIL_HORIZONS)
    ...
    def _apply_moe_topk(self, x_expert_input, gate_input, gate_network, experts_modulelist, k, noise_std):
        ...
        all_out = torch.stack([e(x_expert_input) for e in experts_modulelist], dim=1)  # [B, NumExperts, ExpertOutDim]
        # Correct gathering for batch processing
        # x_expert_input could be [N, Din] or [N, M, Din]
        # all_out will be [N, NumExperts, Dout] or [N, M, NumExperts, Dout]
        # topk_idx will be [N, eff_k] or [N, M, eff_k]
        # We need to gather along the NumExperts dimension.

        # Assuming x_expert_input is [N, Din], all_out is [N, NumExperts, Dout]
        # topk_idx is [N, eff_k]. Need to expand it for gathering.
        gather_idx_shape = list(topk_idx.shape) + [all_out.size(-1)]  # e.g., [N, eff_k, Dout]
        gather_idx = topk_idx.unsqueeze(-1).expand(gather_idx_shape)  # [N, eff_k, Dout]

        # Example: all_out [10, 8, 64], topk_idx [10, 2], gather_idx [10, 2, 64]
        # sel_out should be [10, 2, 64]
        # This means all_out.gather needs dim=1 for expert selection.
        sel_out = all_out.gather(1, gather_idx)  # Dim 1 is the expert dimension in all_out

        y = (sel_out * topk_w.unsqueeze(-1)).sum(dim=1)  # Dim 1 is eff_k dimension after gather

        router_prob_for_loss = torch.softmax(logits, -1)
        avg_router_prob = router_prob_for_loss.mean(0)  # Mean over batch

        # Ensure dimensions match for scatter_add_
        # topk_idx might be [N, eff_k]. We need to scatter into a tensor of size [NumExperts]
        # Flatten topk_idx for scattering, ones_for_scatter should match its new size.
        ...
        ) / (gate_input.size(0) * eff_k)  # gate_input.size(0) is batch size N
    ...
    def forward(self, x_features_globally_std, sensor_mask):
        ...
        # SENSOR_INPUT_DIM is 1, so last dim of x_features_globally_std should be model_max_sensors
        # x_features_globally_std: [Batch, SeqLen, MaxSensors]
        # Need to permute to [Batch, MaxSensors, SeqLen] for TCN processing
        # Then reshape to [Batch*MaxSensors, SeqLen, SENSOR_INPUT_DIM=1]

        x_permuted = x_features_globally_std.permute(0, 2, 1)  # [B, MaxSensors, SeqLen]

        # Create the SENSOR_INPUT_DIM dimension
        x_expanded_for_tcn = x_permuted.unsqueeze(-1)  # [B, MaxSensors, SeqLen, 1]

        x_reshaped_for_encoder = x_expanded_for_tcn.reshape(
            batch_size * self.model_max_sensors, seq_len, SENSOR_INPUT_DIM
        )

        # For last_val_globally_std, it should be [B, MaxSensors]
        last_val_globally_std = x_permuted[:, :, -1].clone()  # [B, MaxSensors]

        sensor_temporal_features_flat = self.per_sensor_encoder(
            x_reshaped_for_encoder)  # [B*MaxSensors, SeqLen, TCN_OUT_DIM]
        ...
        # Apply sensor_mask after reshaping
        ...
        )  # moe_forecast_output: [B, MoeOutputDim]
        moe_fail_output, aux_fail, logits_fail = self._apply_moe_topk(
            ...
            self.moe_top_k, self.moe_noise_std
        )  # moe_fail_output: [B, MoeOutputDim]

        # For RCA, expert input is per-token (per-sensor) context
        ...
        if x_flat_rca_expert_input_valid.size(0) > 0:
            # Gate input for RCA is also per-token
            ...
            moe_rca_output_flat_valid, aux_rca, logits_rca_valid = self._apply_moe_topk(
                ...
                self.moe_top_k, self.moe_noise_std
            )  # moe_rca_output_flat_valid: [NumValidTokens, MoeOutputDim]
        ...
        # moe_rca_output_flat is [B*MaxSensors, MoeOutputDim]
        ...
        # last_val_globally_std is [B, MaxSensors]
        pred_abs_globally_std = last_val_globally_std.unsqueeze(-1) + pred_delta_globally_std
        pred_abs_globally_std = pred_abs_globally_std * sensor_mask.unsqueeze(-1)  # Apply mask
        ...
        ctx_flat = cross_sensor_context_masked.reshape(-1,
                                                       self.transformer_d_model)  # [B*MaxSensors, TransformerDModel]
        # moe_rca_output_flat is already [B*MaxSensors, MoeOutputDim]
...
def train_and_save_model():
    ...
    # Initialize EMA variables for dynamic loss weighting (only for Failure Focal Loss alpha now)
    ema_pos_fail, ema_tot_fail = torch.tensor(0.0, device=DEVICE), torch.tensor(0.0, device=DEVICE)
    # ema_pos_rca, ema_tot_rca removed due to Change 3

    print("Starting multi-task training (Forecast, Fail, RCA) with updated loss configurations...")
    for epoch in range(EPOCHS):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            fail_tgt_orig = batch["fail_targets"].to(DEVICE);  # Original targets
            rca_tgt_orig = batch["rca_targets"].to(DEVICE)  # Original targets

            # Change 4: Light label smoothing (one-sided: 1 -> factor, 0 -> 0)
            fail_tgt = fail_tgt_orig * LABEL_SMOOTH_FACTOR
            rca_tgt = rca_tgt_orig * LABEL_SMOOTH_FACTOR
            ...
            with autocast(enabled=AMP_ENABLED):
                ...
                abs_target_std = last_k_std.unsqueeze(-1) + delta_tgt_std  # Targets are not smoothed for prediction
                ...
                # --- Failure Loss ---
                # Change 1 (Time-decay weight inside failure BCE) is superseded by Change 2 for lf_bce.
                # Change 2: LogSumExp pooling over horizon for lf_bce
                fail_logit_event = torch.logsumexp(fail_logits, dim=1)  # Shape: [B]
                # Use smoothed fail_tgt for consistency if it's used to form fail_tgt_event
                fail_tgt_event = fail_tgt.max(dim=1).values  # Shape: [B] (based on smoothed fail_tgt)

                # BCE for LogSumExp output, without pos_weight as per sketch for Change 2
                lf_bce = F.binary_cross_entropy_with_logits(fail_logit_event, fail_tgt_event)

                # EMA calculations for pos_w_fail and alpha_fail for the *Focal term*
                # These are based on original (pre-smoothing) per-horizon fail_tgt for stability of weights.
                batch_pos_fail_for_ema = fail_tgt_orig.sum().float()
                batch_tot_fail_for_ema = torch.tensor(fail_tgt_orig.numel(), device=DEVICE, dtype=torch.float)
                ema_pos_fail = EMA_BETA * ema_pos_fail + (1 - EMA_BETA) * batch_pos_fail_for_ema
                ema_tot_fail = EMA_BETA * ema_tot_fail + (1 - EMA_BETA) * batch_tot_fail_for_ema
                pos_w_fail = (ema_tot_fail - ema_pos_fail) / (ema_pos_fail.clamp(min=1e-6))
                alpha_fail = pos_w_fail / (1.0 + pos_w_fail.clamp(min=1e-9))  # Clamped denominator for safety

                # Focal loss term operates on the original per-horizon logits and smoothed fail_tgt
                focal_fail_loss_fn = FocalLoss(alpha=alpha_fail.item(), gamma=FAILURE_FOCAL_GAMMA, reduction='mean')
                lf_focal_term = focal_fail_loss_fn(fail_logits, fail_tgt)  # Uses smoothed fail_tgt here

                lf = lf_bce + FAILURE_FOCAL_BLEND_COEFF * lf_focal_term

                # --- RCA Loss ---
                # Change 3: Static class-weight for RCA BCE
                lr_bce = F.binary_cross_entropy_with_logits(rca_logits, rca_tgt,  # rca_tgt is smoothed
                                                            weight=sensor_m,
                                                            pos_weight=torch.tensor(POS_W_RCA, device=DEVICE))

                # Alpha for RCA Focal loss is derived from static POS_W_RCA
                static_alpha_rca = POS_W_RCA / (1.0 + POS_W_RCA)
                focal_rca_loss_fn = FocalLoss(alpha=static_alpha_rca, gamma=RCA_FOCAL_GAMMA, reduction='none')
                lr_focal_elements = focal_rca_loss_fn(rca_logits, rca_tgt)  # rca_tgt is smoothed
        ...
        if valid_loader:
            ...
            with torch.no_grad():
                for batch_val in valid_loader:
                    ...
                    fail_tgt_orig_val = batch_val["fail_targets"].to(DEVICE);  # Original targets
                    rca_tgt_orig_val = batch_val["rca_targets"].to(DEVICE)  # Original targets

                    # Change 4: Light label smoothing for validation targets
                    fail_tgt_val = fail_tgt_orig_val * LABEL_SMOOTH_FACTOR
                    rca_tgt_val = rca_tgt_orig_val * LABEL_SMOOTH_FACTOR

                    with autocast(enabled=AMP_ENABLED):
                        pred_abs_std_val, fail_logits_val, rca_logits_val, l_aux_val, l_entr_val = model(input_feat_val,
                                                                                                         sensor_m_val)

                        abs_target_std_val = last_k_std_val.unsqueeze(-1) + delta_tgt_std_val  # Not smoothed
                        ...
                        # Change 2: LogSumExp for lf_bce_val
                        fail_logit_event_val = torch.logsumexp(fail_logits_val, dim=1)
                        fail_tgt_event_val = fail_tgt_val.max(dim=1).values  # Based on smoothed fail_tgt_val
                        lf_bce_val = F.binary_cross_entropy_with_logits(fail_logit_event_val, fail_tgt_event_val)

                        # Focal term: use current EMA state for alpha_fail (derived from original targets)
                        current_pos_w_fail = (ema_tot_fail - ema_pos_fail) / (ema_pos_fail.clamp(min=1e-6))
                        current_alpha_fail = current_pos_w_fail / (1.0 + current_pos_w_fail.clamp(min=1e-9))
                        focal_fail_loss_fn_val = FocalLoss(alpha=current_alpha_fail.item(), gamma=FAILURE_FOCAL_GAMMA,
                                                           reduction='mean')
                        lf_focal_val = focal_fail_loss_fn_val(fail_logits_val,
                                                              fail_tgt_val)  # Uses smoothed fail_tgt_val
                        lf_val = lf_bce_val + FAILURE_FOCAL_BLEND_COEFF * lf_focal_val

                        # RCA Loss (Validation)
                        # Change 3: Static class-weight for lr_bce_val
                        lr_bce_val = F.binary_cross_entropy_with_logits(rca_logits_val, rca_tgt_val,
                                                                        # rca_tgt_val is smoothed
                                                                        weight=sensor_m_val,
                                                                        pos_weight=torch.tensor(POS_W_RCA,
                                                                                                device=DEVICE))
                        static_alpha_rca_val = POS_W_RCA / (1.0 + POS_W_RCA)
                        focal_rca_loss_fn_val = FocalLoss(alpha=static_alpha_rca_val, gamma=RCA_FOCAL_GAMMA,
                                                          reduction='none')
                        lr_focal_elements_val = focal_rca_loss_fn_val(rca_logits_val,
                                                                      rca_tgt_val)  # rca_tgt_val is smoothed
                        lr_focal_val_term = (lr_focal_elements_val * sensor_m_val).sum() / sensor_m_val.sum().clamp(
                            min=1e-9)
                        lr_val = lr_bce_val + RCA_FOCAL_BLEND_COEFF * lr_focal_val_term

                        val_loss = W_PRED * lp_val + W_FAIL * lf_val + W_RCA * lr_val + l_aux_val + l_entr_val

                        # Change 5: Hysteresis in post-processing (for potential metrics, not affecting loss)
                        fail_probs_val = torch.sigmoid(fail_logits_val)  # [B, NumHorizons]
                        # Initialize state for hysteresis for each item in the batch
                        state_hysteresis = torch.zeros(fail_probs_val.size(0), device=DEVICE)  # [B]
                        output_alarms_hysteresis = torch.zeros_like(fail_probs_val)  # [B, NumHorizons]

                        for t_horizon_idx in range(fail_probs_val.size(1)):  # Iterate over horizon dimension
                            # Current probabilities for this horizon step
                            p_t = fail_probs_val[:, t_horizon_idx]  # [B]

                            # Update state based on sketch: state = torch.where(state==0, (p[:,t]>hi).float(), (p[:,t]>lo).float())
                            is_state_zero = (state_hysteresis == 0)
                            # If state is 0, it becomes 1 if p_t > hi_thresh
                            becomes_one = is_state_zero & (p_t > HYSTERESIS_HI_THRESH)
                            # If state is 1, it remains 1 if p_t > lo_thresh
                            remains_one = (~is_state_zero) & (p_t > HYSTERESIS_LO_THRESH)

                            state_hysteresis = (becomes_one | remains_one).float()
                            output_alarms_hysteresis[:, t_horizon_idx] = state_hysteresis
                        # `output_alarms_hysteresis` now contains the failure predictions after hysteresis.
                        # This can be used for calculating metrics if needed later.
...
if __name__ == '__main__':
    print(
        "--- Script Version: Multi-Task Foundational Model with Refactored MoE (Vectorized Soft Top-k, MMoE, Token RCA, Richer Router, Entropy Reg, Updated Losses) ---")
