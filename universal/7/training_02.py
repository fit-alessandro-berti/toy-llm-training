...
# Training Parameters (Updated based on suggestions)
EPOCHS = 40  # Increased epochs
BATCH_SIZE = 32
LEARNING_RATE = 5e-4  # Adjusted learning rate
ADAM_BETAS = (0.9, 0.98)  # Kept from original
ADAM_WEIGHT_DECAY = 1e-4  # Adjusted weight decay for AdamW
HUBER_DELTA = 1.0

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

MODEL_SAVE_PATH = "simplified_timeseries_model_h5_v2.pth"
PREPROCESSOR_SAVE_PATH = "simplified_preprocessor_config_h5_v2.npz"


# --- RevIN Layer (Will be replaced by nn.Identity in the model if disabled) ---
...
class TemporalBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, dropout=0.1):
        ...
        # Weight norm removed as per suggestion
...
# --- Data Handling (StandardScaler is kept, RevIN will be disabled in model) ---
def get_max_sensors_from_files(file_paths, cap=MAX_SENSORS_CAP):
    max_s = 0
    if not file_paths: print("Warning: No data files found for determining max sensors."); return 0
    for fp in file_paths:
        try:
            ...
            print(f"Warning: Could not read {fp} to determine max sensors: {e}"); continue
    ...
    def _load_and_prepare_data(self):
        ...
        for file_idx, fp in enumerate(self.file_paths):
            ...
            if not sensor_cols:
                ...
                sensor_cols = [c for c in potential_cols if not any(
                    kw in c.lower() for kw in ['time', 'date', 'label', 'failure', 'id', 'current_failure'])]
                if not sensor_cols: print(
                    f"Warning: No 'Sensor*' or inferable sensor columns found in {fp}. Skipping."); continue
...
# --- Simplified Prediction Model (RevIN Disabled) ---
class SimplifiedPredictionModel(nn.Module):
    def __init__(self, max_sensors, seq_len, sensor_input_dim, sensor_tcn_proj_dim, sensor_tcn_out_dim,
                 tcn_levels, tcn_kernel_size, tcn_dropout, revin_affine=True, use_revin=False):  # Added use_revin flag
        super().__init__()
        self.max_sensors = max_sensors;
        self.seq_len = seq_len;
        self.sensor_tcn_out_dim = sensor_tcn_out_dim
        self.use_revin = use_revin  # Store the flag

        if self.use_revin:
            self.revin_layer = RevIN(num_features=max_sensors, affine=revin_affine)
            print("RevIN is ENABLED.")
        else:
            self.revin_layer = nn.Identity()  # RevIN disabled as per suggestion
            print("RevIN is DISABLED (using nn.Identity). StandardScaler from Dataset is primary normalization.")
    ...
    def forward(self, x_features_orig_scale, sensor_mask):  # x_features_orig_scale is already StandardScaler normalized
        batch_size, seq_len, _ = x_features_orig_scale.shape

        # If RevIN were used, it would operate here.
        # x_revin_input = x_features_orig_scale.permute(0, 2, 1) * sensor_mask.unsqueeze(-1) # Mask before RevIN stats
        # if self.use_revin:
        #     x_processed_for_encoder = self.revin_layer(x_revin_input, mode='norm')
        #     last_val_for_delta_recon = x_processed_for_encoder[:, :, -1]
        # else: # RevIN is nn.Identity
        #     x_processed_for_encoder = self.revin_layer(x_revin_input) # Pass through
        #     last_val_for_delta_recon = x_processed_for_encoder[:, :, -1]

        # Simplified: Input to TCN is already StandardScaler normalized if RevIN is Identity
        # The name x_features_orig_scale might be misleading now; it's StandardScaler normalized.
        x_scaled_input = x_features_orig_scale.permute(0, 2, 1) * sensor_mask.unsqueeze(
            -1)  # Apply mask for consistency, though Identity won't use it for stats

        # Apply what would have been RevIN (now Identity or actual RevIN)
        # If nn.Identity, it just passes x_scaled_input.permute(0,2,1)
        # If RevIN, it normalizes x_scaled_input.permute(0,2,1)
        # Let's make it explicit:
        current_normalized_data = self.revin_layer(x_scaled_input) if not self.use_revin else self.revin_layer(
            x_scaled_input, mode='norm')

        last_val_norm_for_delta_recon = current_normalized_data[:, :, -1]  # [B, MaxSensors]

        x_for_encoder = current_normalized_data.permute(0, 2, 1)  # [B, SeqLen, MaxSensors]
        ...
        features_for_pred_head = sensor_temporal_features[:, :, -1, :]  # [B, MaxSensors, TCN_OUT_DIM]
        pred_delta_in_normalized_space = self.pred_head_h5(features_for_pred_head)  # [B, MaxSensors, 1]

        pred_abs_in_normalized_space = last_val_norm_for_delta_recon.unsqueeze(-1) + pred_delta_in_normalized_space

        # Denormalize if RevIN was used, else it's an identity op and values remain StandardScaler-normalized.
        final_pred = self.revin_layer(pred_abs_in_normalized_space,
                                      mode='denorm') if self.use_revin else self.revin_layer(
            pred_abs_in_normalized_space)
        final_pred = final_pred * sensor_mask.unsqueeze(-1)  # Final masking

        return final_pred


# --- Training Function (Loss Averaging Updated) ---
def train_and_save_model():
    ...
    if not all_file_paths: print("ERROR: No CSV files in TRAIN_DIR or VALID_DIR."); return
    max_s_overall = get_max_sensors_from_files(all_file_paths, MAX_SENSORS_CAP)
    if max_s_overall == 0: print("ERROR: max_sensors_overall is 0."); return
    ...
    # Model initialized with use_revin=False by default, disabling RevIN
    model = SimplifiedPredictionModel(
        ...
        use_revin=False  # Explicitly disable RevIN here as per "minimal recipe"
    ).to(DEVICE)
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, batch in enumerate(train_loader):
            input_features = batch["input_features"].to(DEVICE)  # Already StandardScaler normalized
            sensor_mask = batch["sensor_mask"].to(DEVICE)
            last_known = batch["last_known_values_input"].to(DEVICE)  # StandardScaler normalized
            delta_target = batch["pred_delta_target_h5"].to(DEVICE)  # Delta in StandardScaler normalized space

            optimizer.zero_grad()
            # Model predicts absolute values in StandardScaler normalized space (since RevIN is Identity)
            pred_abs_normalized = model(input_features, sensor_mask)

            # Target is also absolute values in StandardScaler normalized space
            abs_target_normalized = last_known.unsqueeze(-1) + delta_target.unsqueeze(-1)

            loss_elements = loss_fn(pred_abs_normalized, abs_target_normalized)  # [B, MaxSensors, 1]

            # Updated loss averaging
            active_loss_elements = loss_elements[sensor_mask.bool().unsqueeze(-1)]  # Select active elements
            if active_loss_elements.numel() > 0:
                batch_loss = active_loss_elements.mean()  # Mean over all active predictions in the batch
            else:  # Should not happen if batch has data
                batch_loss = torch.tensor(0.0, device=DEVICE, requires_grad=True)

            if torch.isnan(batch_loss) or torch.isinf(batch_loss):
                print(f"Warning: NaN/Inf loss at E{epoch + 1}, B{batch_idx + 1}. Skipping update.");
                continue
            ...
            if batch_idx % (len(train_loader) // 5 if len(train_loader) >= 5 else 1) == 0:
                print(
        ...
        if valid_loader:
            ...
            with torch.no_grad():
                for batch in valid_loader:
                    ...
                    loss_elements = loss_fn(pred_abs_normalized, abs_target_normalized)
                    ...
                    if active_loss_elements.numel() > 0:
                        ...
                        if not (torch.isnan(batch_loss) or torch.isinf(batch_loss)):
                            total_valid_loss_epoch += batch_loss.item()
                            num_valid_batches += 1
    ...
    print(f"Training complete. Model saved: {MODEL_SAVE_PATH}, Preprocessor: {PREPROCESSOR_SAVE_PATH}")


if __name__ == '__main__':
    print("---------------------------------------------------------------------")
    print("IMPORTANT: Ensure TRAIN_DIR and VALID_DIR are correctly set.")
    print(f"Current TRAIN_DIR: {TRAIN_DIR}, Current VALID_DIR: {VALID_DIR}")
    print("If these are not your data directories, please edit the script.")
    print("Minimal recipe changes applied: RevIN disabled, no weight_norm, LR=5e-4, new loss avg, epochs=40.")
    print("---------------------------------------------------------------------")
    if BASE_DATA_DIR == "../../data/time_series/1":
        print("\nWARNING: Using default example data paths. These might not exist.\n")
