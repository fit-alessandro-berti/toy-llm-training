...
MOE_EXPERT_INPUT_DIM = TRANSFORMER_D_MODEL  # Input dim for the Expert MLPs
MOE_HIDDEN_DIM_EXPERT = 128
MOE_OUTPUT_DIM = 64
MOE_TOP_K = 2
MOE_NOISE_STD = 1.0  # Will be effectively 0 during model.eval() due to self.training check
AUX_LOSS_COEFF = 0.01  # Not used for loss calculation in test, but part of model def
ENTROPY_REG_COEFF = 0.01  # Not used for loss calculation in test, but part of model def
...
MODEL_LOAD_PATH = "foundation_multitask_model_v3_moe_vectorized.pth"  # <<< Path to your latest trained model
PREPROCESSOR_LOAD_PATH = "foundation_multitask_preprocessor_v3.npz"  # <<< Path to your latest preprocessor
...
class TestMultivariateTimeSeriesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        item = self.data_cache[file_idx]  # Corrected variable name
        ...
        for k_idx in range(num_to_copy):  # Renamed k to k_idx
...
class FoundationalTimeSeriesModel(nn.Module):
    def __init__(self, model_max_sensors, seq_len,
                 ...
                 moe_top_k, moe_noise_std, aux_loss_coeff, entropy_reg_coeff):  # Updated signature
        super().__init__()
    ...
    def _apply_moe_topk(self, x_expert_input, gate_input, gate_network, experts_modulelist, k, noise_std):
        ...
        if self.training and noise_std > 0:  # Check self.training for eval mode
    ...
    def forward(self, x_features_globally_std, sensor_mask):
        ...
        moe_forecast_output, aux_f, logits_f = self._apply_moe_topk(
            ...
            self.moe_top_k, self.moe_noise_std if self.training else 0.0  # Pass 0 noise if not training
        )
        moe_fail_output, aux_fail, logits_fail = self._apply_moe_topk(
            ...
            self.moe_top_k, self.moe_noise_std if self.training else 0.0
        )
        ...
        if self.entropy_reg_coeff > 0:  # Also ensure self.training if entropy loss is only for training
...
# --- Test Script Logic ---
def test_model():
    print(f"--- Test Script for Foundational Multi-Task MoE Model (Latest Version) ---")
    ...
    model = FoundationalTimeSeriesModel(
        ...
        moe_top_k=MOE_TOP_K, moe_noise_std=MOE_NOISE_STD,  # noise_std is for training, model.eval() handles it
        aux_loss_coeff=AUX_LOSS_COEFF, entropy_reg_coeff=ENTROPY_REG_COEFF  # Not used in test loss
    ).to(DEVICE)
    ...
    try:
        ...
        print(f"ERROR loading model state: {e}. Check architecture match. Exiting.");
        return
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(test_loader):  # Renamed batch to batch_data
            ...
            # Model forward pass
            # Aux/Entropy losses are returned but not used for printing in test script
            ...
            for i in range(num_samples_to_print):
                ...
                try:
                    ...
                    rca_eval_condition = pred_fail_probs[idx_rca_horizon] > 0.5 or gt_fail_status[
                        idx_rca_horizon] == 1.0
                except ValueError:  # If rca_failure_lookahead is not in fail_horizons
                    print(
                        f"      Warning: RCA_FAILURE_LOOKAHEAD ({loaded_rca_lookahead}) not found in FAIL_HORIZONS ({loaded_fail_horizons}). Cannot determine RCA condition based on failure prediction.")
                    rca_eval_condition = True  # Default to showing RCA if horizon mapping fails
