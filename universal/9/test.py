...
SEQ_LEN = 64
PRED_HORIZONS = [1, 3, 5]
# FAIL_HORIZONS will be loaded from preprocessor
# RCA_FAILURE_LOOKAHEAD will be loaded from preprocessor
...
MOE_OUTPUT_DIM = 64
MOE_TOP_K = 2
MOE_NOISE_STD = 0.3    # Suggestion 6 from training script
AUX_LOSS_COEFF = 0.01  # Not used for loss calculation in test, but part of model def
ENTROPY_REG_COEFF = 0.001 # Suggestion 6 from training script

# Testing Params
TEST_BATCH_SIZE = 4 # Reduced for potentially larger models / memory
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
MAX_BATCHES_TO_TEST = 5
SAMPLES_PER_BATCH_TO_PRINT = 2

# Adjusted paths to match latest training script output
MODEL_LOAD_PATH = "foundation_multitask_model_v4_suggestions.pth"
PREPROCESSOR_LOAD_PATH = "foundation_multitask_preprocessor_v4.npz"
...
class PerSensorEncoderTCN(nn.Module):
    def __init__(self, input_dim, proj_dim, tcn_out_dim, seq_len, num_levels, kernel_size, dropout):
        ...
        self.cls_token = nn.Parameter(torch.randn(1, 1, proj_dim)) # Learnable CLS token
        self.pos_encoder = PositionalEncoding(proj_dim, max_len=max(seq_len + 1, 5000)) # Accommodate CLS
    ...
    def forward(self, x):  # x: [Batch*MaxSensors, SeqLen, InputDim]
        ...
        h_cls_normed = self.final_norm(h_cls.unsqueeze(1)).squeeze(1) # Norm CLS output as well
...
class TestMultivariateTimeSeriesDataset(Dataset):
    def __init__(self, data_dir, seq_len, pred_horizons, fail_horizons, rca_failure_lookahead,
        ...
        self.fail_horizons = fail_horizons # Loaded from preprocessor
        self.rca_failure_lookahead = rca_failure_lookahead # Loaded from preprocessor
    ...
    def _load_and_prepare_data(self):
        ...
        for file_idx, fp in enumerate(self.file_paths):
            ...
            # Use loaded/dynamic horizons for max_lookahead
            max_pred_h = max(self.pred_horizons) if self.pred_horizons else 0
            max_fail_h = max(self.fail_horizons) if self.fail_horizons else 0
            max_lookahead = max(max_pred_h, max_fail_h, self.rca_failure_lookahead)
    ...
    def __getitem__(self, idx):
        ...
        last_known_normed = padded_input_normed[-1, :].copy() # Used for novelty and delta targets
...
# --- Foundational Multi-Task Model (from latest training script - with CLS and RCA Novelty) ---
class FoundationalTimeSeriesModel(nn.Module):
    def __init__(self, model_max_sensors, seq_len,
        ...
        rca_head_input_dim = sensor_tcn_out_dim + transformer_d_model + moe_output_dim + 1 # +1 for novelty delta
    ...
    def _apply_moe_topk(self, x_expert_input, gate_input, gate_network, experts_modulelist, k, noise_std):
        ...
        if self.training and noise_std > 0: # self.training is False in eval mode
        ...
        # Aux loss components (load balancing) - not strictly needed for inference output but part of MoE logic
        ...
        return y, load_balance_loss, logits # Return logits for potential entropy calculation
    ...
    def forward(self, x_features_globally_std, sensor_mask, last_known_values_globally_std_for_novelty):
        ...
        # MoE noise is effectively 0 if self.training is False
        moe_noise_to_pass = self.moe_noise_std if self.training else 0.0
        ...
        moe_forecast_output, aux_f, logits_f = self._apply_moe_topk(
            ...
            self.moe_top_k, moe_noise_to_pass
        )
        ...
        total_aux_loss = self.aux_loss_coeff * (aux_f + aux_fail + aux_rca) # For completeness, not used in test loss

        total_entropy_loss = torch.tensor(0.0, device=x_features_globally_std.device)
        # Entropy loss is primarily for training, but computation depends on logits
        ...
        last_val_observed_for_pred = x_features_globally_std[:, -1, :] # Assuming SENSOR_INPUT_DIM=1
        ...
        # RCA Novelty
        with torch.no_grad(): # Ensure no gradients for this part if called during training for some reason
            pred_next_h1 = pred_abs_globally_std[:, :, 0] # Prediction for H=1

        # last_known_values_globally_std_for_novelty is [B, MS]
...
# --- Test Script Logic ---
def test_model():
    print(f"--- Test Script for Foundational Multi-Task MoE Model (Version 4 Suggestions) ---")
    ...
    try:
        ...
        # Use .get() for new keys that might not be in older preprocessor files
        loaded_fail_horizons = list(preprocessor_data.get('fail_horizons', [3, 5, 10])) # Default if not found
        loaded_rca_lookahead = int(preprocessor_data.get('rca_failure_lookahead', loaded_fail_horizons[0] if loaded_fail_horizons else 3))


        if loaded_seq_len != SEQ_LEN:
            print(f"Warning: SEQ_LEN mismatch! Loaded: {loaded_seq_len}, Script: {SEQ_LEN}. Using loaded: {loaded_seq_len}")
            # script_seq_len = loaded_seq_len # Prefer loaded value
        if list(loaded_pred_horizons) != PRED_HORIZONS:
             print(f"Warning: PRED_HORIZONS mismatch! Loaded: {loaded_pred_horizons}, Script: {PRED_HORIZONS}. Using loaded: {loaded_pred_horizons}")
            # script_pred_horizons = list(loaded_pred_horizons) # Prefer loaded value

    except FileNotFoundError:
        print(f"ERROR: Preprocessor file not found at {PREPROCESSOR_LOAD_PATH}. Exiting.");
        return
    except KeyError as e:
        print(f"ERROR: Missing key {e} in preprocessor file. Ensure it's from the v4 training. Exiting.");
        return

    print(
        f"Preprocessor loaded: model_max_sensors_dim={model_max_sensors_dim}, {len(canonical_sensor_names)} canonical sensors.")
    print(f"Using Fail Horizons: {loaded_fail_horizons}, RCA Lookahead: {loaded_rca_lookahead}")


    # 2. Initialize Dataset and DataLoader
    if not (os.path.exists(VALID_DIR) and os.path.isdir(VALID_DIR)):
        print(f"ERROR: Test data directory '{VALID_DIR}' missing or not a directory."); return

    test_dataset = TestMultivariateTimeSeriesDataset(
        data_dir=VALID_DIR, seq_len=loaded_seq_len, # Use loaded SEQ_LEN
        pred_horizons=loaded_pred_horizons, # Use loaded PRED_HORIZONS
    ...
    # 3. Initialize Model
    # Ensure all architectural parameters match the training script for v4
    model = FoundationalTimeSeriesModel(
        model_max_sensors=model_max_sensors_dim, seq_len=loaded_seq_len, # Use loaded SEQ_LEN
        ...
        pred_horizons_len=len(loaded_pred_horizons), # Use loaded PRED_HORIZONS
    ...
    try:
        ...
        print(f"ERROR: Model file not found: {MODEL_LOAD_PATH}. Exiting.");
        return
    except RuntimeError as e: # Catch potential architecture mismatches
        print(f"ERROR loading model state: {e}. This often indicates an architecture mismatch between the saved model and the current script. Ensure this test script matches the training script that produced '{MODEL_LOAD_PATH}'. Exiting.");
        return
    except Exception as e:
        print(f"ERROR loading model state (general error): {e}. Exiting.");
        return
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(test_loader):
            ...
            last_k_std_gt = batch_data["last_known_values_globally_std"].to(DEVICE) # For novelty and targets
            delta_tgt_std_gt = batch_data["pred_delta_targets_globally_std"].to(DEVICE)
            fail_tgt_gt = batch_data["fail_targets"].to(DEVICE)
            rca_tgt_gt = batch_data["rca_targets"].to(DEVICE)

            # Pass last_k_std_gt for novelty calculation within the model's forward pass
            pred_abs_globally_std, fail_logits, rca_logits, _, _ = model(
            ...
            for i in range(num_samples_to_print):
                ...
                for h_idx, horizon in enumerate(loaded_pred_horizons): # Use loaded horizons
                    print(f"      H={horizon}:")
                    for s_local_idx, s_global_model_idx in enumerate(
                            active_sensor_indices[:min(3, len(active_sensor_indices))]): # Print for first few active sensors
                ...
                for h_idx, horizon in enumerate(loaded_fail_horizons): # Use loaded horizons
                    print(
                ...
                # Ensure loaded_rca_lookahead is in loaded_fail_horizons to find its index
                try:
                    # Find the index corresponding to the RCA lookahead horizon in the fail_logits/probs
                    idx_rca_horizon = loaded_fail_horizons.index(loaded_rca_lookahead)
                    rca_eval_condition = pred_fail_probs[idx_rca_horizon] > 0.5 or gt_fail_status[idx_rca_horizon] == 1.0
                except ValueError:
                    print(f"      Warning: RCA_FAILURE_LOOKAHEAD ({loaded_rca_lookahead}) not found in FAIL_HORIZONS ({loaded_fail_horizons}). Cannot determine RCA condition based on failure prediction. Showing RCA scores by default.")
                    rca_eval_condition = True # Default to show if mapping fails
                except IndexError:
                     print(f"      Warning: Index error looking up RCA horizon. Pred_fail_probs length: {len(pred_fail_probs)}, Fail_tgt length: {len(gt_fail_status)}, Idx_rca_horizon: {idx_rca_horizon if 'idx_rca_horizon' in locals() else 'undefined'}. Showing RCA by default.")
                ...
                if rca_eval_condition:
                    ...
                    for s_local_idx, s_global_model_idx in enumerate(
                            active_sensor_indices[:min(5, len(active_sensor_indices))]): # Print for first few active sensors
                        s_name = canonical_sensor_names[s_global_model_idx] if s_global_model_idx < len(
                            canonical_sensor_names) else f"Sensor Pad {s_global_model_idx + 1}"
