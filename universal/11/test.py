...
# --- Configuration (MUST MATCH LATEST TRAINING SCRIPT v6_survival_enhanced) ---
# Data paths
...
TCN_LEVELS = 4
TCN_KERNEL_SIZE = 3
TCN_DROPOUT = 0.1  # Dropout is typically active during training only; model.eval() handles it.
...
# Adjusted paths to match latest training script output (v6_survival_enhanced)
MODEL_LOAD_PATH = "foundation_multitask_model_v6_survival_enhanced.pth"
PREPROCESSOR_LOAD_PATH = "foundation_multitask_preprocessor_v6_survival_enhanced.npz"
...
class TestMultivariateTimeSeriesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {"input_features": torch.from_numpy(padded_input_normed), "sensor_mask": torch.from_numpy(sensor_mask),
                ...
                "eval_fail_targets_binary": torch.from_numpy(eval_fail_targets_binary),
...
def test_model():
    ...
    test_dataset = TestMultivariateTimeSeriesDataset(
        ...
        eval_fail_horizons=loaded_eval_fail_horizons,
