...
# --- Paths ---
MODEL_LOAD_PATH = "foundation_multitask_model_v3_moe_vectorized_cost_sensitive.pth"  # <<< Updated Path
PREPROCESSOR_LOAD_PATH = "foundation_multitask_preprocessor_v3.npz"
...
class TestMultivariateTimeSeriesDataset(Dataset):
    def __init__(self, data_dir, seq_len, pred_horizons, fail_horizons, rca_failure_lookahead,
        ...
        self.fail_horizons = fail_horizons # Will use value loaded from preprocessor
        self.rca_failure_lookahead = rca_failure_lookahead # Will use value loaded from preprocessor
    ...
    def _load_and_prepare_data(self):
        ...
        for file_idx, fp in enumerate(self.file_paths):
            ...
            failure_flags = np.zeros(len(df), dtype=np.int64) # Default to no failures
            ...
            self.data_cache.append({
                ...
                "filepath": fp # Store filepath for richer output
            })
            # Ensure max_lookahead uses the actual fail_horizons and rca_failure_lookahead for this dataset instance
    ...
    def __getitem__(self, idx):
        ...
        item_data = self.data_cache[file_idx] # Use item_data consistently
        ...
        for k_idx_sensor in range(num_to_copy): # Use a distinct name for sensor index
            if not np.all(np.isnan(input_slice_normed[:, k_idx_sensor])):
                sensor_mask[k_idx_sensor] = 1.0
        padded_input_normed[np.isnan(padded_input_normed)] = 0.0 # Impute NaNs after masking
        ...
        if end_r <= len(flags_full) and np.any(flags_full[start_r:end_r]): # If any failure in RCA lookahead window
            ...
            for k_idx_sensor in range(num_to_copy):
                if sensor_mask[k_idx_sensor] > 0: # Only for active sensors
                    ...
                    if len(valid_current_raw) > 0 and len(valid_future_raw) > 0:
                        ...
                        # Check for significant deviation in the future window
        ...
        return {"input_features": torch.from_numpy(padded_input_normed), "sensor_mask": torch.from_numpy(sensor_mask),
                ...
                "pred_delta_targets_globally_std": torch.from_numpy(delta_targets_normed), # Ground truth for comparison
...
class FoundationalTimeSeriesModel(nn.Module):
    def __init__(self, model_max_sensors, seq_len,
        ...
        self.aux_loss_coeff = aux_loss_coeff # Part of def, not used in eval loss
        self.entropy_reg_coeff = entropy_reg_coeff # Part of def, not used in eval loss
    ...
    def _apply_moe_topk(self, x_expert_input, gate_input, gate_network, experts_modulelist, k, noise_std):
        ...
        if self.training and noise_std > 0: # noise_std is effectively 0 if not self.training
        ...
        # Aux losses (load balance) are computed even in eval if desired for inspection,
        # but typically not added to a primary "eval_loss" metric.
        ...
        ) / (x_expert_input.size(0) * eff_k if x_expert_input.size(0) > 0 else 1.0) # Avoid div by zero if batch is empty
        load_balance_loss = self.num_shared_experts * (avg_router_prob * expert_frac).sum()
        return y, load_balance_loss, logits # Return logits for potential inspection

    def forward(self, x_features_globally_std, sensor_mask):
        batch_size, seq_len_dim, _ = x_features_globally_std.shape # Use seq_len_dim to avoid conflict
        ...
        # During eval (self.training is False), noise_std passed to _apply_moe_topk will be 0
        current_noise_std = self.moe_noise_std if self.training else 0.0

        moe_forecast_output, aux_f, _ = self._apply_moe_topk( # logits_f not typically used in eval output
            mean_ctx_global, router_input_global, self.gates["forecast"], self.experts_shared,
            self.moe_top_k, current_noise_std
        )
        moe_fail_output, aux_fail, _ = self._apply_moe_topk( # logits_fail not typically used in eval output
        ...
        # Initialize outputs for RCA MoE
        ...
        if x_flat_rca_expert_input_valid.size(0) > 0:
            x_flat_rca_gate_input_valid = x_flat_rca_expert_input_valid # For token-level, gate input is same as expert input
            moe_rca_output_flat_valid, aux_rca_valid, _ = self._apply_moe_topk( # logits_rca not typically used in eval output
            ...
            aux_rca = aux_rca_valid # Assign the computed aux loss

        # Aux losses are part of model's forward pass by definition, but usually ignored in eval script's primary metrics
        total_aux_loss = self.aux_loss_coeff * (aux_f + aux_fail + aux_rca)
        total_entropy_loss = torch.tensor(0.0, device=x_features_globally_std.device) # Usually 0 if not training
        ...
        # Return aux/entropy loss for completeness, though they might not be used in test script's evaluation
        return pred_abs_globally_std, fail_logits, rca_logits, total_aux_loss, total_entropy_loss


# --- Test Script Logic ---
def test_model():
    print(f"--- Test Script for Foundational Multi-Task MoE Model (Cost-Sensitive Trained Version) ---") # Updated Version
    ...
    try:
        ...
        # Load sequence and horizon parameters from preprocessor to ensure consistency
        loaded_seq_len = int(preprocessor_data.get('seq_len', SEQ_LEN))
        loaded_pred_horizons = list(preprocessor_data.get('pred_horizons', PRED_HORIZONS))
        loaded_fail_horizons = list(preprocessor_data.get('fail_horizons', FAIL_HORIZONS))
        loaded_rca_lookahead = int(preprocessor_data.get('rca_failure_lookahead', RCA_FAILURE_LOOKAHEAD))

        # Check for mismatches with script defaults, primarily for user awareness
        if loaded_seq_len != SEQ_LEN:
            print(f"Warning: SEQ_LEN mismatch! Loaded from preprocessor: {loaded_seq_len}, Script default: {SEQ_LEN}. Using loaded value.")
        if loaded_pred_horizons != PRED_HORIZONS:
            print(f"Warning: PRED_HORIZONS mismatch! Loaded: {loaded_pred_horizons}, Script default: {PRED_HORIZONS}. Using loaded value.")
        # Similar checks for fail_horizons and rca_lookahead can be added if strict adherence to script defaults is critical
    ...
    test_dataset = TestMultivariateTimeSeriesDataset(
        data_dir=VALID_DIR,
        seq_len=loaded_seq_len, # Use loaded value
        pred_horizons=loaded_pred_horizons, # Use loaded value
        fail_horizons=loaded_fail_horizons, # Use loaded value
        rca_failure_lookahead=loaded_rca_lookahead, # Use loaded value
    ...
    if len(test_dataset) == 0:
        ...
        return
    test_loader = DataLoader(test_dataset, batch_size=TEST_BATCH_SIZE, shuffle=False, num_workers=0) # num_workers=0 for simplicity in debugging

    # 3. Initialize Model (using potentially loaded config values for dimensions)
    model = FoundationalTimeSeriesModel(
        model_max_sensors=model_max_sensors_dim,
        seq_len=loaded_seq_len, # Use loaded value
        sensor_input_dim=SENSOR_INPUT_DIM, # Architectural, should match training
        ...
        pred_horizons_len=len(loaded_pred_horizons), # Use loaded value
        fail_horizons_len=len(loaded_fail_horizons), # Use loaded value
        moe_top_k=MOE_TOP_K,
        moe_noise_std=MOE_NOISE_STD, # Will be 0 in eval mode due to model's internal check
        aux_loss_coeff=AUX_LOSS_COEFF, # Part of model def, not used for eval metric
        entropy_reg_coeff=ENTROPY_REG_COEFF # Part of model def, not used for eval metric
    ).to(DEVICE)
    ...
    with torch.no_grad():
        for batch_idx, batch_content in enumerate(test_loader): # Renamed to batch_content
            ...
            # Model forward pass
            # Aux/Entropy losses are returned by model's forward but typically not used for evaluation metrics here
            pred_abs_globally_std, fail_logits, rca_logits, _, _ = model(input_globally_std, sensor_m)

            actual_abs_targets_std = last_k_std_gt.unsqueeze(-1) + delta_tgt_std_gt # Reconstruct absolute GT
            ...
            for i in range(num_samples_to_print_this_batch):
                ...
                for h_idx, horizon_val in enumerate(loaded_pred_horizons): # Iterate with loaded horizons
                    print(f"      Horizon = {horizon_val} steps:")
                    # Print for a few active sensors
                    for s_local_idx, s_global_model_idx in enumerate(active_sensor_indices[:min(3, len(active_sensor_indices))]): # Print for first 3 active sensors
                ...
                for h_idx, horizon_val in enumerate(loaded_fail_horizons): # Iterate with loaded horizons
                ...
                # Determine if RCA is "active" based on failure prediction for the specific RCA lookahead horizon
                # This requires finding the index of rca_lookahead in fail_horizons
                rca_active_based_on_failure = False
                try:
                    rca_horizon_idx_in_fail_list = loaded_fail_horizons.index(loaded_rca_lookahead)
                    if gt_fail_status[rca_horizon_idx_in_fail_list] == 1.0 or pred_fail_probs[rca_horizon_idx_in_fail_list] > 0.5 : # Example threshold
                    ...
                    rca_active_based_on_failure = True # Default to showing if direct link fails

                if rca_active_based_on_failure:
                    pred_rca_scores = torch.sigmoid(rca_logits[i]).cpu().tolist()
                    # Print for a few active sensors
                    for s_local_idx, s_global_model_idx in enumerate(active_sensor_indices[:min(5, len(active_sensor_indices))]): # Print for first 5 active sensors
                        sensor_name_display = canonical_sensor_names[s_global_model_idx] if s_global_model_idx < len(canonical_sensor_names) else f"Sensor_Pad_{s_global_model_idx+1}"
