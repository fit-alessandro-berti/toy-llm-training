...
# Model & Training Parameters (from V5 training script)
SEQ_LEN = 64
PRED_HORIZONS = [1, 3, 5]
...
NUM_EXPERTS_PER_TASK = 8  # Must match V5 training
MOE_HIDDEN_DIM_EXPERT = 128
MOE_OUTPUT_DIM = 64
# MOE_TOP_K is effectively 1 in V5's _apply_moe_switch
EXPERT_DROPOUT_RATE = 0.1  # Used in model def, but dropout is off in eval mode
...
MODEL_LOAD_PATH = "foundation_timeseries_model_v5.pth"  # V5 model
PREPROCESSOR_LOAD_PATH = "preprocessor_config_v5.npz"  # V5 preprocessor


# --- RevIN Layer (from V5 training script) ---
    ...
    def forward(self, x, mode):  # x: [Batch, NumSensors, SeqLen]
        if mode == 'norm':
            ...
            if not hasattr(self, 'mean') or not hasattr(self, 'stdev'):
                # Fallback for safety if stats weren't computed (e.g. during a failed forward pass on an empty batch)
                # print("Warning: RevIN denorm called without valid stats, returning input as is.") # Can be verbose
                return x
...
# --- Helper: CausalConv1D Block (from V5 training script) ---
...
# --- Data Handling (Replicated from V5 training script) ---
...
class MultivariateTimeSeriesDataset(Dataset):
    ...
    def _load_and_prepare_data(self):
        ...
        for file_idx, fp in enumerate(self.file_paths):
            ...
            scalers = [StandardScaler() for _ in range(num_actual_sensors)]  # Per-file, per-sensor scaling
...
# --- Model Architecture (Replicated from V5 training script) ---
...
class FoundationalTimeSeriesModelV5(nn.Module):
    def __init__(self, max_sensors, seq_len,
                 ...
                 # moe_top_k is implicitly 1
                 pred_horizons_len, fail_horizons_len, revin_affine=True):
        super().__init__()
        self.max_sensors = max_sensors
        self.seq_len = seq_len
        # self.moe_top_k = 1 # Implicit for Switch Router
        self.num_experts_per_task = num_experts_per_task  # Store this
    ...
    def _apply_moe_switch(self, global_moe_input, gating_network, expert_pool):
        ...
        router_probs = torch.softmax(gating_logits, dim=-1)  # For aux loss
        ...
        # Expert dropout is applied during training. model.eval() handles it for test.
        moe_task_output = self.expert_dropout(moe_task_output)

        fi = one_hot_selection.mean(dim=0)
        Pi = router_probs.mean(dim=0)
        return moe_task_output, fi, Pi  # fi, Pi are for aux loss, not strictly needed for inference pass

    def forward(self, x_features_orig_scale,
                sensor_mask):  # Removed apply_revin_denorm_to_pred, model always denorms preds
        batch_size, seq_len, _ = x_features_orig_scale.shape
        x_revin_input = x_features_orig_scale.permute(0, 2, 1) * sensor_mask.unsqueeze(
            -1)  # Apply mask before stat calculation
        ...
        moe_forecast_output, _, _ = self._apply_moe_switch(global_moe_input, self.gating_forecast,
                                                           self.experts_forecast)  # Ignore fi, Pi for inference
        moe_fail_output, _, _ = self._apply_moe_switch(global_moe_input, self.gating_fail, self.experts_fail)
        moe_rca_output, _, _ = self._apply_moe_switch(global_moe_input, self.gating_rca, self.experts_rca)

        aux_loss_terms_dummy = {"forecast": (None, None), "fail": (None, None),
                                "rca": (None, None)}  # For consistent return signature
        ...
        # During inference, we don't need aux_loss_terms for loss calculation
        return pred_absolute_final_denorm, fail_output_logits, rca_output_logits  # Modified return for test script
...
def test_model():
    ...
    model = FoundationalTimeSeriesModelV5(  # Using V5 class
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(valid_loader):
            ...
            input_features_orig_scale = batch["input_features"].to(DEVICE)  # Original scale (or dataset-scaled)
            ...
            # Model's forward pass now includes RevIN and returns denormalized absolute predictions
            pred_absolute_final_denorm, fail_output_logits, rca_output_logits = model(input_features_orig_scale,
                                                                                      sensor_mask)
            # Note: The V5 model's forward used to return aux_loss_terms too. For inference, it's cleaner if it doesn't.
            # I've adjusted the V5 model's forward in this script to only return main outputs for inference.

            # Ground truth absolute predictions for comparison
            ...
            for i in range(num_samples_this_batch_to_print):
                ...
                for h_idx, horizon_steps in enumerate(PRED_HORIZONS):
                    ...
                    for s_global_idx in sensors_to_print_forecast:
                        ...
                        # Predicted delta is not directly output by V5 model in this inference setup, so cannot easily print it without re-normalizing
                        print(
                ...
                idx_rca_horizon = FAIL_HORIZONS.index(
                    RCA_FAILURE_LOOKAHEAD)  # Ensure RCA_FAILURE_LOOKAHEAD is in FAIL_HORIZONS
